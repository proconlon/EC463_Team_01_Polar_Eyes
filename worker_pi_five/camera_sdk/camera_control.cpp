...
// #region agent log
// Debug instrumentation. Writes NDJSON lines to stderr; the SSH terminal
// transcript captured by the IDE is what the assistant reads back.
static inline void dbgLog(const char* location, const char* message,
                          const std::string& data_json) {
    auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(
        std::chrono::system_clock::now().time_since_epoch()).count();
    std::cerr << "[DBG-2bd714] {\"sessionId\":\"2bd714\",\"timestamp\":"
              << ms << ",\"location\":\"" << location
              << "\",\"message\":\"" << message
              << "\",\"data\":" << data_json << "}" << std::endl;
}
static inline std::string dbgKV(const std::string& kv) { return "{" + kv + "}"; }
// #endregion
...
public:
    ...
    bool discoverAndConnect() {
        ...
        // #region agent log
        {
            ...
            os << "\"hypothesisId\":\"H1,H4\",\"duration_ms\":" << dt
               << ",\"is_connected\":" << (camera_->IsConnected() ? "true" : "false");
            dbgLog("connect:open", "Camera Open() returned", dbgKV(os.str()));
        }
        // #endregion
    ...
    bool takePhoto(const std::string& save_directory = "./") {
        ...
        // #region agent log
        dbgLog("takePhoto:entry", "takePhoto invoked",
               dbgKV("\"runId\":\"iter11\",\"hypothesisId\":\"H100\",\"isConnected\":true"));
        // #endregion
        ...
        // #region agent log
        auto t_photo_start = std::chrono::steady_clock::now();
        // #endregion
        ins_camera::MediaUrl url = camera_->TakePhoto();
        // #region agent log
        {
            auto dt = std::chrono::duration_cast<std::chrono::milliseconds>(
                std::chrono::steady_clock::now() - t_photo_start).count();
            std::ostringstream os;
            os << "\"runId\":\"iter11\",\"hypothesisId\":\"H100\",\"empty\":"
               << (url.Empty() ? "true" : "false")
               << ",\"single_origin\":" << (url.IsSingleOrigin() ? "true" : "false")
               << ",\"duration_ms\":" << dt;
            dbgLog("takePhoto:take_photo", "TakePhoto result",
                   dbgKV(os.str()));
        }
        // #endregion
    ...
    bool stopRecording(const std::string& save_directory = "./") {
        ...
        {
            auto dt = std::chrono::duration_cast<std::chrono::milliseconds>(
                std::chrono::steady_clock::now() - t_stop_start).count();
            std::ostringstream os;
            os << "\"runId\":\"iter9\",\"hypothesisId\":\"H70,H80\",\"empty\":"
               << (url.Empty() ? "true" : "false")
               << ",\"single_origin\":" << (url.IsSingleOrigin() ? "true" : "false")
               << ",\"duration_ms\":" << dt;
            dbgLog("stopRecording:stop_recording",
                   "StopRecording returned", dbgKV(os.str()));
        }
        // #endregion
